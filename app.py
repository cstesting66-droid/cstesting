    ...
    if request.method == "POST":
        ...
        if not entered:
            ...
            if not valid:
                ...
                cursor.execute(
                    "UPDATE users SET email_verified = 1, status = CASE WHEN phone_verified = 1 THEN 'active' ELSE 'pending' END "
                    "WHERE email = %s",
                    (email,)
                )
                conn.commit()
    ...
    if request.method == "POST":
        ...
        if not entered:
            ...
            if not valid:
                ...
                cursor.execute(
                    "UPDATE users SET phone_verified = 1, status = CASE WHEN email_verified = 1 THEN 'active' ELSE 'pending' END "
                    "WHERE email = %s",
                    (email,)
                )
                conn.commit()
